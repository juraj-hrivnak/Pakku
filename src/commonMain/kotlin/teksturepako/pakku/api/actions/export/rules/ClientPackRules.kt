...
fun exportClientPack() = ExportRule {
    when (it)
    {
        is ExportingProject         ->
        {
            if (OverrideType.fromProject(it.project) !in listOf(OverrideType.OVERRIDE, OverrideType.CLIENT_OVERRIDE))
            {
                it.ignore()
            }
            else
        ...
        {
            if (it.type !in listOf(OverrideType.OVERRIDE, OverrideType.CLIENT_OVERRIDE))
            {
                it.ignore()
            }
            else
        ...
        {
            if (it.projectOverride.type !in listOf(OverrideType.OVERRIDE, OverrideType.CLIENT_OVERRIDE))
            {
                it.ignore()
            }
            else
